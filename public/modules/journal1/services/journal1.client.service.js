...
angular.module( 'journal1' )
	.factory('Journal1', ['$resource',
		function($resource) {
			return $resource('api/journal1/:journalId', {
				journalId: '@_id',
				articleNo:'@articleNo',
				info: '@info'
			}, {
		...
		function ($http, _) {
			var transform = function (data) {
				return _.map(data, function (cat) {
					return {
						text: cat._id,
						weight: cat.count,
						link: '#!/journal1?category='+ _.trim(cat._id)
					};
				});	
			};
			...
			return {
				top: function (done, err) {
					$http.get('/api/journal1/categories')
	...
	.factory( 'DeleteArticle', ['$http',
		function( $http ){
			return {
				query: function( journalId, articleId, cbf ){
					$http.delete( 'api/article1', { params: { articleId: articleId, journalId: journalId } } )
					.success( function( data ){
						if( cbf ){
