/**
	Server-Side controller for Journal1
*/
...
exports.create = function( req, res, next ){
	var journal = new Journal1( req.body );
	var i = 0;
...
/*	var tt = Journal1.findById( req.journal1._id ).exec( function( err, jrnl ){
		console.log( 'gggg  ', jrnl );
	} );*/
	Journal1.update( { _id: req.journal1._id }, { $inc: { views: 1 } } ).exec( function( err ){
		if( err ){
			res.status( 500 ).send( 'Error update views count' );
		}else{
			var jr = req.journal1.toObject();
			jr.user = req.journal1.populated( 'user' );
			...
			//jr.content.author = req.journal1.articles[articleNo || 0].populate( 'author', 'displayName' );
...
exports.search = function( req, res ){
	...
	querySetup = function( hasSufficientRole ){
		...
		if( 'journal1' in req.query ){
			var authors = _.isArray( req.query.journal1.articles )?
				{$in: _.map( req.query.journal1.articles, function( authorId ){
					return mongoose.Types.ObjectId( authorId );
				})}
				: mongoose.Types.ObjectId( req.query.journal1.arcticles );
 ...
 */
exports.canAccessJournal = function( req, res, next ){
	/*jshint ignore:start*/
	if( req.journal1.status === 'open' || req.user && ( req.journal1.populated( 'user' ) == req.user.id ) ){
		return next();
	}else{
		policy.hasAtLeastRole( req, 'reviewer', function( err, has ){
			if( has ) return next();
...
exports.canModifyJournal = function( req, res, next ){
	/*jshint ignore:start*/
	if( req.user && ( req.journal1.populated( 'user' ) == req.user.id ) ){
		next();
	}else{
		policy.hasAtLeastRole( req, 'reviewer', function( err, has ){
			if( has ) next();
...
exports.hasAuthorization = function( req, res, next ){
	if( req.journal1.user.id !== req.user.id ){
...
exports.deleteArticle = function( req, res, next ){
	...
	Journal1.findById( journalId ).exec( function( err, data ){
		var i = 0;
		var index = -1;
		if( err ){
			return next( err );
